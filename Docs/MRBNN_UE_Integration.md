...
- `AMRBNNVolumeActor`: a ready-to-drop volume actor. It exposes the data/baking, cloud shape, density, lighting, baked-feature, and quality controls directly on the actor, bakes the active MRBNN density field and spatial baked feature files into transient `UVolumeTexture` objects, drives the default SceneViewExtension compute composite path, synchronizes the scene directional light into per-actor relight parameters, and can aggregate a bounded scene-light summary for the shader.
...
Launch arguments are `WorkDir`, `MRBNNRoot`, and `PluginRoot`; if they are omitted, the console defaults to the bundled `cloud-03` data. Inside the console, a top-bar language selector switches between English and Simplified Chinese. The tabbed workflow separates Setup, Render, Cloud Bake / Sync, and Actions: Setup selects the MRBNN working directory, repository root, plugin root, scene name, HDRI, and skybox-baking folder; Render controls CUDA reference output size, sample count, camera, light direction/color, albedo, phase, tonemapping, denoise, skybox, and skybox baking; Actions initializes/releases the CUDA renderer, renders a single sample or averaged frame, saves PPM plus raw RGBA32F buffers, and packages the selected data into plugin `Data/`.

The Cloud Bake / Sync tab bakes a local CUDA Cloud Info volume without duplicating Unreal actor Details controls. It can keep the original source density or apply a reproducible cloud-shape generator preset (`source_volume`, `cumulus_core`, `anvil_tower`, `layer_bank`, `wispy_streaks`) with seed, coverage, base height, thickness, edge softness, detail, and wind shear knobs. A headless equivalent is available:

```powershell
Engine/Plugins/Experimental/MRBNN/Binaries/ThirdParty/MRBNNBridge/Win64/MRBNNBakeConsole.exe `
  --bake-cloud-info `
  --cloud-info-resolution 64 `
  --cloud-shape-generator anvil `
  --cloud-shape-seed 888
```

The bake writes `cloud-03_cloud_info.rgba32f`, `cloud-03_cloud_info_manifest.json`, and the UE handoff file `mrbnn_bake_sync_manifest.json`. The sync manifest records schema version, scene name, source working directory, plugin root, output directory, Cloud Info paths, generator type/seed/shape params, bake resolution, density source metadata, timestamps, and optional UE copy targets. By default it is also copied to the active work directory so the editor import command can find it.

In Unreal, the editor-only MRBNN toolbar adds `MRBNN` to launch the console and `Sync MRBNN` to import the latest bake. The sync command reads `mrbnn_bake_sync_manifest.json` from Project Settings (or falls back to `<DefaultDataRoot>/<DefaultSceneName>/mrbnn_bake_sync_manifest.json`), packages the source working directory into plugin `Data/<scene>`, copies Cloud Info sidecars when present, rejects manifest copy targets that resolve outside the MRBNN plugin root, updates MRBNN Project Settings, and refreshes MRBNN actors when enabled.
...
`FMRBNNSceneViewExtension` subscribes to the Tonemap post-process pass, lets one leader extension gather all visible `MRBNNVolumeActor` instances in the world, asks each actor for a per-view camera/render description, dispatches `MainCS` into per-cloud RDG textures, then alpha-composites them into one `MRBNN.CombinedSceneCloud` texture before `MainCompositeCS` blends that cloud layer over the current SceneColor. The single plugin shader file performs bounded volume traversal in the plugin-built 3D textures, a small shadow traversal along the scene directional light, a controllable Henyey-Greenstein-style phase response, baked-feature proxy lighting, edge silver lining, deep-volume absorption, powder-style cloud scattering, bounded scene-light contribution, multi-cloud cloud-over composition, RT debug display, and the final scene composite. The old cube-mesh material raymarch path has been removed from the runtime actor.

This is closer to the paper than a density-only cloud because it uses the paper's baked spatial radiance features and now runs through UE's render graph instead of a material preview. It is still an approximation: the full paper path samples `base`, `ms`, `view`, `light`, `hg`, `albedo`, and transmittance features and evaluates the trained MLP/TCNN decoder. The bounded scene-light path is also approximate. It reads public scene actor/component state on the game thread, ranks nearby local point/spot/rect lights by estimated influence at the cloud actor, sends at most four to the shader, and evaluates analytic distance/cone/area terms per raymarch sample. It samples the first visible skylight, exponential-height-fog color/density, and SkyAtmosphere scalar art-direction values as ambient/fog lift. It does not integrate UE shadow maps, SkyAtmosphere LUTs, volumetric fog voxel grids, IES profiles, light functions, rect source textures, reflection captures, or per-pixel renderer light lists. The optional native bridge remains the closest implementation of the full neural decoder and now emits raw float buffers for parity checks; the default compute path is the game-friendly UE shader path that can run without CUDA.
...
- `MRBNN|Lighting`: scene `DirectionalLight` binding, ambient/direct balance, intensity scale, optional bounded scene lighting, local light count/intensity, sky/atmosphere/fog scale, shadow steps/density, phase, silver lining, deep shadow, and powder scattering.
...
For a sky-filling mode that can replace or complement UE's Volumetric Cloud component, see `Docs/MRBNN_Sky_RT_Architecture.md`. That planned path introduces a separate sky cloud component, a sky-layer shader domain, and a shared render-source interface instead of adding more responsibilities to `AMRBNNVolumeActor`.
